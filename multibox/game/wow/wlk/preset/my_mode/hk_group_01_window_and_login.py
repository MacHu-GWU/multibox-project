...
class HotkeyGroup01WindowAndLoginMixin:
    def build_hk_round_robin_toggle_window(self: "Mode"):
        with hk.Hotkey(
    ...
    def build_hk_toggle_specific_window(self: "Mode"):
        """
        定义切换到指定窗口的快捷键.
    ...
    def build_hk_center_overlap_layout(self: "Mode"):
        with hk.Hotkey(
    ...
    def build_hk_login_specific_account(self: "Mode"):
        """
        定义在指定窗口登录指定账号的快捷键.
    ...
    def build_hk_batch_logout(self: "Mode"):
        with hk.Hotkey(
            id="BatchLogout",
            ...
            with hk.SendLabel(
                ...
                # 点击关闭菜单按钮
                hk.MoveMouse(
                    x=self.client.return_to_game_button_x,
                    y=self.client.return_to_game_button_y,
                )
                hk.Wait.make(50)
                ...
                # 点击登出按钮
                hk.MoveMouse(
                    x=self.client.log_out_button_x,
                    y=self.client.log_out_button_y,
                )
                hk.Wait.make(50)
                hk.ClickMouse.make_left_click_on_window()

    def build_hk_logout_on_current_window(self: "Mode"):
        with hk.Hotkey(
    ...
    def build_hk_group_01_window_and_login_mixin(self: "Mode"):
