...
class HotkeyGroup07UtilitySpellMixin:
    def build_hk_domino_action_bar_5(self: "HknScript"):
        ...
        #     with hk.SendLabel(
        #         to=self.mode.team.lbs_by_tc(TC.shaman),
        #     ):
        #         hk.Key.trigger()
    ...
    def build_hk_domino_action_bar_4(self: "HknScript"):
        ...
        with hk.Hotkey(
            ...
            for label in self.mode.team.lbs_by_tc(TC.druid_balance):
                with hk.Toggle():
                    with hk.SendLabel(
                        to=[
                            label,
    ...
    def build_hk_domino_action_bar_3(self: "HknScript"):
        ...
        with hk.Hotkey(
            ...
            with hk.SendLabel(
                id=TC.dk.name,
                to=self.mode.team.lbs_by_tc(TC.dk),
            ):
                # act.General.ESC()
            ...
            with hk.SendLabel(
                id=TC.hunter.name,
                to=self.mode.team.lbs_by_tc(TC.hunter),
            ):
                # act.General.ESC()
                act.Hunter.Volley()
            ...
            with hk.SendLabel(
                ...
                to=self.mode.team.lbs_by_tc(TC.druid_balance),
            ):
                # act.General.ESC()
            ...
            with hk.SendLabel(
                id=TC.warlock.name,
                to=self.mode.team.lbs_by_tc(TC.warlock),
            ):
                # act.General.ESC()
            ...
            with hk.SendLabel(
                id=TC.mage.name,
                to=self.mode.team.lbs_by_tc(TC.mage),
            ):
                # act.General.ESC()
                act.Mage.Blizzard()
    ...
    def build_hk_domino_action_bar_2(self: "HknScript"):
        ...
        #         to=self.mode.team.lbs_by_tc(TC.paladin_protect),
        #     ):
        #         hk.Key(key=KN.R)
        #
        #
        ...
        #         to=self.mode.team.lbs_by_tc(TC.paladin_holy),
        #         actions=[
        ...
        #         to=self.mode.team.lbs_by_tc(TC.dk_tank),
        #         actions=[
        ...
        #     SendLabel(
        #         name=TC.dk_dps.name,
        #         to=self.mode.team.lbs_by_tc(TC.dk_dps),
        #         actions=[
        ...
        #         to=self.mode.team.lbs_by_tc(TC.hunter_marksman),
        #         actions=[
        #             act.Target.TARGET_FOCUS_TARGET,
        ...
        #     self.mode.team.lbs_by_tc(TC.tank),
        #     special_labels,
        # )
        #
        # regular_dps_labels = difference_list(
        #     self.mode.team.lbs_by_tc(TC.dps),
        #     special_labels,
        # )
        #
        # regular_healer_labels = difference_list(
        #     self.mode.team.lbs_by_tc(TC.healer),
        #     special_labels,
        # )
        #
        ...
        with hk.Hotkey(
            ...
            # 近战打断, CD 都是 10 秒
            # warrior_lbs = self.mode.team.lbs_by_tc(TC.warrior)
            # rogue_lbs = self.mode.team.lbs_by_tc(TC.rogue)
            dk_lbs = self.mode.team.lbs_by_tc(TC.dk)

            # 远程打断
            shaman_non_resto_lbs = self.mode.team.lbs_by_tc(
                TC.shaman_non_resto
            )  # 萨满打断技能 CD 很短, 只有 6 秒
            shaman_resto_lbs = self.mode.team.lbs_by_tc(
                TC.shaman_resto
            )  # 萨满打断技能 CD 很短, 只有 6 秒
            shaman_lbs = shaman_non_resto_lbs + shaman_resto_lbs
            mage_lbs = self.mode.team.lbs_by_tc(TC.mage)  # 法师反制 CD 24 秒
            marksman_hunter_lbs = self.mode.team.lbs_by_tc(
                TC.hunter_marksman
            )  # 猎人 沉默射击 CD 24 秒
            # shadow_priest_lbs = self.mode.team.lbs_by_tc(TC.priest_shadow) # 不是每一个牧师都会点出沉默
        ...
        with hk.Hotkey(
            ...
            with hk.SendLabel(
                id=TC.dispeler.name,
                to=self.mode.team.lbs_by_tc(TC.dispeler),
            ):
