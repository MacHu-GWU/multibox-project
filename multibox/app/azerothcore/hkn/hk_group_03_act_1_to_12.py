...
class HotkeyGroup03Act1To12Mixin:
    ...
    def build_actions_default(
        ...
        **注**
        ...
        for talent in TC.tank.talents:
            with hk.SendLabel(
                id=talent.name,
                to=self.mode.team.lbs_by_tl(talent),
            ) as send_label:
                hk.Key.make(key)
    ...
    def build_hk_1_heal_tank(self: "HknScript"):
        with hk.Hotkey(
            ...
            # 奶德, 奶萨 用小治疗奶坦克
            label_list_druid_resto = self.mode.team.lbs_by_tc(TC.druid_resto)
            label_list_shaman_resto = self.mode.team.lbs_by_tc(TC.shaman_resto)
            ...
            if len(label_list) == 0:
                ...
                with hk.SendLabel(
                    ...
                    self.mode.team.target_leader_1()
                    hk.Key.make(KN.KEY_1)
                with hk.SendLabel(
                    id="SlowTankHealer2",
                    to=[
                    ...
                    self.mode.team.target_leader_2()
                    hk.Key.make(KN.KEY_1)
                with hk.SendLabel(
    ...
    def build_hk_3_heal_tank(self: "HknScript"):
        with hk.Hotkey(
            ...
            label_list = self.mode.team.lbs_by_tc(TC.paladin_healer)
            if len(label_list) == 0:
                pass
            # 如果只有 1 个奶骑, 则给焦点加圣光道标
                ...
                with hk.SendLabel(
                    ...
                    to=self.mode.team.lbs_by_tc(TC.paladin_healer),
                ):
                    act.Target.TARGET_FOCUS()
    ...
    def build_hk_4_heal_nothing(self: "HknScript"):
        with hk.Hotkey(
            ...
            label_list = self.mode.team.lbs_by_tc(TC.shaman_resto)
            if len(label_list) == 0:
                pass
            # 如果只有 1 个奶萨, 则给焦点补大地之盾
                ...
                with hk.SendLabel(
                    ...
                    to=self.mode.team.lbs_by_tc(TC.shaman_resto),
                ):
                    act.Target.TARGET_FOCUS()
                    hk.Key.make(KN.KEY_4)
            ...
            label_list = self.mode.team.lbs_by_tc(TC.priest_disco)
            if len(label_list) == 0:
                pass
            # 如果只有 1 个戒律牧, 则给焦点补愈合祷言
                ...
                with hk.SendLabel(
                    id=TC.shaman_resto.name,
                    to=self.mode.team.lbs_by_tc(TC.priest_disco),
                ):
                    act.Target.TARGET_FOCUS()
                    hk.Key.make(KN.KEY_4)
    ...
    def build_hk_0_short_term_buff(self: "HknScript"):
        ...
        with hk.Hotkey(
            ...
            with hk.SendLabel(
                id=TC.priest.name,
                to=self.mode.team.lbs_by_tc(TC.priest),
            ):
    ...
    def build_hk_11_focus_mode_1(self: "HknScript"):
        ...
        with hk.Hotkey(
            ...
            for char in self.mode.team.active_chars:
    ...
    def build_hk_alt_5(self: "HknScript"):
        ...
        with hk.Hotkey(
            ...
            with hk.SendLabel(
                ...
                to=self.mode.team.lbs_by_tc(tc=TC.priest_holy),
            ):
                act.Target.TARGET_SELF()
            ...
            with hk.SendLabel(
                id=TC.shaman.name,
                to=self.mode.team.lbs_by_tc(tc=TC.shaman),
            ):
                act.Target.TARGET_FOCUS_TARGET()
                act.Shaman.Chain_Heal()
