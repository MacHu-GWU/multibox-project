...
from multibox.game.wow.wlk.talent import (
    TL,
    TC,
    OrderedSet,
)
...
class TestTalentCategory:
    ...
    def test_sortable(self):
        talent_category_list = [TC.tank, TC.dps, TC.healer]
...
def test_get_category_by_talent():
    ...
    assert TL.druid_pve_bear.is_pve() is True
    assert TL.druid_pvp_cat.is_pve() is False

    assert TL.druid_pve_bear.is_pvp() is False
    assert TL.druid_pvp_cat.is_pvp() is True

    assert TL.druid_pve_bear.is_tank() is True
    assert TL.druid_pvp_cat.is_tank() is False

    assert TL.druid_pve_bear.is_dps() is False
    assert TL.druid_pvp_cat.is_dps() is True

    assert TL.druid_pve_bear.is_healer() is False
    assert TL.druid_pvp_cat.is_healer() is False

    assert TL.druid_pve_bear.is_melee() is True
    assert TL.druid_pvp_cat.is_melee() is True

    assert TL.druid_pve_bear.is_ranger() is False
    assert TL.druid_pvp_cat.is_ranger() is False

    assert TL.druid_pve_bear.is_physics() is True
    assert TL.druid_pvp_cat.is_physics() is True

    assert TL.druid_pve_bear.is_caster() is False
    assert TL.druid_pvp_cat.is_caster() is False

    assert TL.druid_pve_bear.is_dispeler() is False
    assert TL.druid_pvp_cat.is_dispeler() is False


if __name__ == "__main__":
    from multibox.tests import run_cov_test

    run_cov_test(__file__, "multibox.game.wow.wlk.talent", preview=False)
